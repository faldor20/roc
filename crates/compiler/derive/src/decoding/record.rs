...
/// Makes the vars for decoding this particular field
fn make_decoder_var(
    env: &mut Env<'_>,
    field_var: Variable,
    decode_err_var: Variable,
...
fn decode_with(
    ...
    let (decode_rec_dot_result, decode_rec_var, decoder_var, lambda_set_var, this_decode_with_var) =
        make_decoder_var(
            env,
            field_var,
            decode_err_var,
    ...
    (condition_expr, decode_rec_var, decode_rec_dot_result)
}
...
fn attempt_expression(
    ...
    let decode_when = {
        let (partial_expr, rec_result, rec_dot_result) = decode_with(
            env,
            field_var,
            empty_list(Variable::LIST_U8),
            fmt_arg_var,
            fmt_arg_symbol,
            decode_err_var,
        );
